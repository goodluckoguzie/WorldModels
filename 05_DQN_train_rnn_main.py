...
import numpy as np
from hparams import DQNRNNHyperParams as hp
...
class RNN_MODEL():
    ...
    def configure(self, config:str):
        ...
        if self.run_name is not None:
            self.writer = SummaryWriter('DQNRNN_model_runs/'+self.run_name)
        else:
    ...
    def train(self):
        ...
        for idx in range(1, self.max_step + 1):        # while self.global_step < self.max_step:
            ...
            if self.global_step % self.save_interval == 0:
                ...
                torch.save(
                    d, os.path.join(self.ckpt_dir, '{:03d}dqnrnn.pth.tar'.format(self.global_step//self.save_interval))
                )
...
if __name__ == '__main__':
    ...
    config = "./configs/DQNRNN_model.yaml"
        # declaring the network
    Agent =RNN_MODEL(config, run_name="DQNRNN_model_runs")
