...
split_idx = 5_000
train_data = dict(list(train_data.items())[:split_idx])

val_data = torch.load('./Data/saved_vae_rollout_validation.pt')
val_data = dict(list(val_data.items())[:split_idx ])
...
class LSTM(nn.Module):
    ...
    def __init__(self, n_latents,n_actions, n_hiddens, n_layers):            
        ...
        self.lstm = nn.LSTM(input_size=n_latents+n_actions,hidden_size=n_hiddens, num_layers=n_layers,batch_first=True,dropout=0.70)
...
config = "./configs/RNN_hidden_256_layer_1.yaml"
    # declaring the network
Agent = RNN_LSTM(config, run_name="RNN_hidden_256_layer_1_win5")
