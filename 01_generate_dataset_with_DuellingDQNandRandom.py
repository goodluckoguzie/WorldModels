...
class DuelingDQNAgent:
    ...
    def get_action(self, current_state, epsilon):
        ...
        self.duelingDQN.load_state_dict(torch.load('./models/duelingdqn_testbasealivereward/episode00100000.pth'))
...
def rollout():
    ...
    for ep in range(max_ep):
        obs_lst, action_lst, reward_lst, next_obs_lst, done_lst,Treward = [], [], [], [], [],[]
        obs = env.reset()
        ...
        for t in range(time_steps+2):       
            # env.render()
            L = [0, 1]
            action_selection = random.choice(L)
            ...
            if done:
                ...
                Treward = torch.tensor(Treward).unsqueeze(-1)
                # reward_lst = pad_tensor(reward_lst, episode_length=time_steps).cpu().detach().numpy()
                # reward_lst=torch.from_numpy(reward_lst)
                break
