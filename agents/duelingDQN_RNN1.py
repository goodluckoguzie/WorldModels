...
class DuelingDQNAgent:
    def __init__(self, env:gym.Env, config:str, **kwargs) -> None:
        ...
        self.rnn = RNN(n_latents_rnn, n_actions, n_hiddens, n_layers).to(device)
        ...
        self.ckpt  = sorted(glob.glob(os.path.join(self.ckpt_dir, 'RobotFrameDatasetsTimestep1window_16', '010robotframemain.pth.tar')))[-1] #
        # self.ckpt  = sorted(glob.glob(os.path.join(self.ckpt_dir, 'RobotFrameDatasetsTimestep1window_32', '010robotframemain.pth.tar')))[-1] #
    ...
    def train(self):
        ...
        for i in range(self.num_episodes):
            ...
            # print("2222",z.shape)
            # print("11111",hidden[0].squeeze(0).shape)
            ...
            while not done: 
                ...
                with torch.no_grad():
                    ...
                    # rnn_input = torch.cat([unsqueezed_z.unsqueeze(0).to(self.device), unsqueezed_action.to(self.device)], dim=-1).float()
                    # rnn_input = torch.cat([unsqueezed_z.unsqueeze(0).to(self.device), unsqueezed_action.to(self.device)], dim=-1).float()
