    ...
    def train(self):
        ...
        for i in range(self.num_episodes):
            ...
            while not done: 
                ...
                #     current_obs_X_2_ = current_obs_X_2_[-1, :]
                #     rnn_input_X_2_ = torch.cat([current_obs_X_2_.unsqueeze(0).unsqueeze(0), unsqueezed_action.to(self.device)], dim=-1).float()
                #     # rnn_input_X_2_ = torch.cat([next_obs, unsqueezed_action.to(self.device)], dim=-1).float()

                #     _,_, _, hidden = self.rnn.infer(rnn_input_X_2_,hidden)

                # # next_obs = torch.cat((next_obs.to(self.device), hidden[0].to(self.device)), -1)
                # next_obs = torch.cat((z.unsqueeze(0).unsqueeze(0), hidden[0].unsqueeze(0)),-1)
...
if __name__ == "__main__":
    ...
    input_layer_size = 23 #env.observation_space["goal"].shape[0] + env.observation_space["humans"].shape[0] + env.observation_space["laptops"].shape[0] + env.observation_space["tables"].shape[0] + env.observation_space["plants"].shape[0]+hiddens
