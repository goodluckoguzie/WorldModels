...
class DuelingDQNAgent:
    def __init__(self, env: gym.Env, config: str, **kwargs) -> None:
        ...
        rnn_state = torch.load(ckpt, map_location={'cuda:0': str(self.device)})
    ...
    def get_action(self, current_state, epsilon):
        ...
        if np.random.random() > epsilon:
            ...
            with torch.no_grad():
                ...
                q = self.duelingDQN(current_state.unsqueeze(
                    0)).reshape(1, -1).float().to(self.device)
    ...
    def train(self):
        ...
        for i in range(self.num_episodes):
            ...
            print(
                f"Episode {i+1} Reward: {self.episode_reward} Loss: {self.episode_loss}")
            self.plot(i+1)
            ...
            if (self.save_path is not None) and ((i+1) % self.save_freq == 0) and self.episode_reward >= self.average_reward:
                ...
                try:
                    self.save_model(os.path.join(
                        self.save_path, "episode" + str(i+1).zfill(8) + ".pth"))
                except:
            ...
            if (i+1) % self.save_freq == 0:
                ...
                self.average_reward = (
                    (i % self.save_freq)*self.average_reward + self.episode_reward)/((i % self.save_freq)+1)
...
if __name__ == "__main__":
    ...
    config = "./configs/dqn_trained_rnn_duelingDQN.yaml"
