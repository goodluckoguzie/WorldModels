...
# Define a Wall class to store wall properties and corner points
class Wall:
...
# Add a helper function to extract corner points from a Wall object
...
# Modify the add_walls_as_obstacles function
def add_walls_as_obstacles(sim, walls):
    for wall in walls:
        wall_points = get_wall_points(wall)
        sim.addObstacle([(point[0], point[1]) for point in wall_points])
...
# Main loop
speed_factor = 1.0  # You can change this value to control the agent's speed
...
#################################################################################
    ...
    while not done:
        ...
        goal_obs, robot_obs, humans_obs = transform_processed_observation_into_raw(preprocess_observation(raw_obs))
        ...
        radius = 2.0
        maxSpeed = 2.0
        sim = PyRVOSimulator(1.0, neighborDist, timeHorizon, timeHorizonObst, radius, maxSpeed, 2.0)

        # Add the robot as an agent to the simulator
        robot = sim.addAgent(tuple((robot_obs[0], robot_obs[1])), 0.3, 10, 1.0, 5.0, 2.0, 180.0, (0, 0))

        # # Add humans to the RVO2 simulator and set their positions and velocities
        # humans = []
        human_size_factor = 1.5  # Increase the size of the human obstacles

        def update_human_positions(humans_obs):
            updated_humans = []
            for i, hum in enumerate(humans_obs):
                updated_pos = np.array(hum[:2]) 
                updated_hum = np.hstack((updated_pos, hum[2:]))
                updated_humans.append(updated_hum)
            return updated_humans

        humans_obs = update_human_positions(humans_obs)
        for i, hum in enumerate(humans_obs):
            human_pos = hum[0], hum[1]
            sim.addObstacle([(human_pos[0] - 0.5 * human_size_factor, human_pos[1] - 0.5 * human_size_factor),
                            (human_pos[0] + 0.5 * human_size_factor, human_pos[1] - 0.5 * human_size_factor),
                            (human_pos[0] + 0.5 * human_size_factor, human_pos[1] + 0.5 * human_size_factor),
                            (human_pos[0] - 0.5 * human_size_factor, human_pos[1] + 0.5 * human_size_factor)])
        sim.processObstacles()


        # Add walls as obstacles to the simulator
        add_walls_as_obstacles(sim, env.walls)
        
        # Calculate direction to goal relative to the robot's position
        goal_direction = np.array(goal_obs) - np.array(robot_obs[:2])

        # Normalize the goal direction vector
        goal_direction_norm = goal_direction / np.linalg.norm(goal_direction)

        # Multiply the normalized goal direction vector by the speed factor
        goal_direction = tuple(goal_direction_norm * speed_factor)

        angle = math.atan2(goal_direction[1], goal_direction[0])
        relative_angle = angle - env.robot.orientation
        sim.setAgentPrefVelocity(robot, (np.cos(relative_angle), np.sin(relative_angle)))

        # Perform a simulation step.
        sim.doStep()

        # Get the new velocity (action) from the simulator
        action = sim.getAgentVelocity(robot)
        ...
        # tm[0,2] = robot_obs[0]
        # tm[1,2] = robot_obs[1]
        # tm[0,0] = tm[1,1] = np.cos(env.robot.orientation)
        # tm[1,0] = np.sin(env.robot.orientation)
        # tm[0,1] = -1*np.sin(env.robot.orientation)
        # coord_in_robot_frame = (tm@np.array([[0], [0], [1]]).T).T
        # print("XXXX", robot_obs[0], robot_obs[1], env.robot.orientation)
        # print("coord_in_robot_frame)
        next_raw_obs, reward, done, info = env.step(np.array([action[0], action[1]]))
...
############################################################################
        ...
        # print('next_raw_obs[goal]', next_raw_obs["goal"])
...
###############################################################################
    ...
    print("Episode [{}/{}] finished after {} timesteps".format(ep+
...
df.to_csv('RESULTS/RVO2Averagesresults.csv', index=False)

# Save the DataFrame to a JSON file
df.to_json('RESULTS/RVO2Averagesresults.json', orient='records')
...
data.to_csv('RESULTS/RVO2results.csv', index=False)

# Save DataFrame to json
data.to_json('RESULTS/RVO2results.json', orient='records')
