...
class DuelingDQNAgent:
    def __init__(self, env:gym.Env, config:str, **kwargs) -> None:
        ...
        self.rnn_1 = RNN(n_latents, n_actions, n_hiddens).to(device)
        self.ckpt  = sorted(glob.glob(os.path.join(self.ckpt_dir, 'RobotFrameDatasetsTimestep05window_16', '018robotframe.pth.tar')))[-1] #
        rnn_state_1 = torch.load(  self.ckpt, map_location={'cuda:0': str(device)})
        self.rnn_1.load_state_dict(rnn_state_1['model'])
        self.rnn_1.eval()
...
########################################################################################################
                    ...
                    _,_, _, rnn_1_hidden = self.rnn_1.infer(rnn_input.unsqueeze(0),rnn_1_hidden)                


                ################################################################################################

                # current_obs = torch.cat((z.unsqueeze(0).unsqueeze(0), hidden[0].unsqueeze(0)),-1)
                current_obs = torch.cat((z.unsqueeze(0), hidden[0],rnn_1_hidden[0]), -1)
                ...
                with torch.no_grad():
                    ...
                    _,_, _, rnn_1_hidden = self.rnn_1.infer(rnn_input.to(self.device),rnn_1_hidden)                
                    # print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",next_obs.shape)
                    # print("111111111111111111111111111111111111111111",unsqueezed_action.shape)
                    # print("222222222222222222222222222222222222",unsqueezed_action.shape)
                next_obs = torch.cat((next_obs.to(self.device), hidden[0].to(self.device),rnn_1_hidden[0]), -1)
...
if __name__ == "__main__":
    ...
    config = "./configs/duelingDQNDoubleTimestepRNN.yaml"
    input_layer_size = env.observation_space["goal"].shape[0] + env.observation_space["humans"].shape[0] + env.observation_space["laptops"].shape[0] + env.observation_space["tables"].shape[0] + env.observation_space["plants"].shape[0]+hiddens+hiddens
