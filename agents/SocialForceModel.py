...
speed_factor = 1.0  # to control the agent's speed
AGENT_GOAL_ATTRACTION_FORCE = 2.5
HUMAN_REPULSION_FORCE = 4.0
...
def distance_to_segment(p, a, b):
    ...
    vec_bp = p - b
    if np.dot(vec_bp, -vec_ab) <= 0:
        return np.linalg.norm(vec_bp)

    cross_product = np.cross(vec_ab, vec_ap)
    return abs(cross_product) / np.linalg.norm(vec_ab)

def social_force_model(robot_pos, goal_pos, humans, walls, speed_factor, 
                       goal_attraction_force=AGENT_GOAL_ATTRACTION_FORCE,
                       human_repulsion_force=HUMAN_REPULSION_FORCE,
                       wall_repulsion_force=HUMAN_REPULSION_FORCE,
                       distance_threshold=DISTANCE_THRESHOLD,
                       max_turning_angle=np.pi / 8,
                       consider_human_orientation=False):
    # Calculate the attractive force towards the goal
    goal_direction = goal_pos - robot_pos
    angle = math.atan2(goal_direction[1], goal_direction[0])
    relative_angle = angle - env.robot.orientation

    # # Limit the turning angle
    # if relative_angle > max_turning_angle:
    #     relative_angle = max_turning_angle
    # elif relative_angle < -max_turning_angle:
    #     relative_angle = -max_turning_angle

    # Calculate the new velocity based on the attractive force and relative angle
    new_velocity = np.array([math.cos(relative_angle), math.sin(relative_angle)])

    # Apply the speed factor and goal attraction force
    new_velocity = new_velocity * speed_factor * goal_attraction_force

    # Calculate the repulsive force from humans
    ...
    for human in humans:
        human_pos = np.array([human[0], human[1]])
        human_to_robot = robot_pos - human_pos
        distance = np.linalg.norm(human_to_robot)
        if distance < closest_distance:
            closest_distance = distance
            closest_human = human

    # Update the repulsive force using the closest human
    if closest_human is not None:
        human_pos = np.array([closest_human[0], closest_human[1]])
        human_to_robot = robot_pos - human_pos
        distance = np.linalg.norm(human_to_robot)

        if distance > 0 and distance < distance_threshold:
            if consider_human_orientation:
                human_angle = math.atan2(human_to_robot[1], human_to_robot[0])
                angle_diff = abs(human_angle - env.robot.orientation)

                if angle_diff < 2 * math.pi / 1:
                    repulsive_force += (human_to_robot / distance) * human_repulsion_force
            else:
                repulsive_force += (human_to_robot / distance) * (human_repulsion_force / distance)

        # Add tangential force
        tangential_force = np.array([-human_to_robot[1], human_to_robot[0]])
        tangential_force = tangential_force / np.linalg.norm(tangential_force)
        repulsive_force += tangential_force #* human_repulsion_force
    ...
    #     print("human[1]",human[1])        
    #     print("robot_pos[0]",robot_pos[0])
    #     print("robot_pos[1]",robot_pos[1])
    #     if distance > 0 and distance < distance_threshold:
    #         if consider_human_orientation:
    #             # human_angle = human[2]
    #             human_angle = math.atan2(human[2], human[3])
    #             # human_angle = math.atan2(human_to_robot[1], human_to_robot[0])
    #             angle_diff = abs(human_angle - env.robot.orientation)

    #             # Only apply repulsive force if human is facing the robot
    #             if angle_diff < math.pi / 2:
    #                 repulsive_force += (human_to_robot / distance) * human_repulsion_force #(human_repulsion_force / distance) * (human_to_robot / distance)
    #         else:
    #             repulsive_force += (human_to_robot / distance) * (human_repulsion_force / distance) 

    # Calculate the repulsive force from walls
    for wall in walls:
        p1, p2 = np.array(wall.p1), np.array(wall.p2)
        distance = distance_to_segment(robot_pos, p1, p2)
        # print("wall distance ",distance )

        if distance > 0 and distance < distance_threshold - 2:
            wall_to_robot = robot_pos - (p1 + (np.dot(robot_pos - p1, p2 - p1) / np.dot(p2 - p1, p2 - p1)) * (p2 - p1))
            repulsive_force += (wall_repulsion_force / distance) * (wall_to_robot / distance)

    # Combine attractive and repulsive forces
    new_velocity += repulsive_force
...
#################################################################################
    ...
    while not done:
        ...
        action = social_force_model(robot_pos, goal_pos, humans, env_walls, speed_factor)
        next_raw_obs, reward, done, info= env.step(action)

        obs = preprocess_observation(next_raw_obs)
...
##########################################################################
        ...
        goal_obs, robot_obs, humans_obs = transform_processed_observation_into_raw(torch.from_numpy(preprocess_human_observation(next_raw_obs))) 
...
df.to_csv('RESULTS/SFMAveragesresults.csv', index=False)

# Save the DataFrame to a JSON file
df.to_json('RESULTS/SFMAveragesresults.json', orient='records')
...
data.to_csv('RESULTS/SFMresults.csv', index=False)

# Save DataFrame to json
data.to_json('RESULTS/SFMresults.json', orient='records')

# Plot results
plt.figure(figsize=(10, 6))
plt.suptitle('Line Plots for Episode Metrics - Social Force Model', fontsize=16)
