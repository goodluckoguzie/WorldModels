...
class SocNavEnv(gym.Env):
    ...
    def render_obs(robot_obs, goal_obs, humans_obs, window_name, world_image=None, dont_draw=False):
        if world_image is None:
            world_image = np.zeros((int(RESOLUTION),int(RESOLUTION),3))
        ...
        def draw_oriented_point(image, input_data, colour, radius=0.15, nose=0.1):
            left =   np.array([input_data[0]+np.cos(input_data[2]+np.pi/2)*radius, input_data[1]+np.sin(input_data[2]+np.pi/2)*radius])
            right =  np.array([input_data[0]+np.cos(input_data[2]-np.pi/2)*radius, input_data[1]+np.sin(input_data[2]-np.pi/2)*radius])
            front =  np.array([input_data[0]+np.cos(input_data[2])*nose,           input_data[1]+np.sin(input_data[2])*nose])
            centre = np.array([input_data[0],                                        input_data[1]])
            ...
            if not dont_draw:
                ...
                cv2.resizeWindow(window_name, int(RESOLUTION_VIEW), int(RESOLUTION_VIEW))
                #     self.window_initialised = True

        # empty the image
        world_image = (np.ones((int(RESOLUTION),int(RESOLUTION),3))*255).astype(np.uint8)
        # draws robot
        draw_oriented_point(world_image, robot_obs, (255, 0, 0))
        # draws goal
        cv2.circle(world_image, (w2px(goal_obs[0]), w2py(goal_obs[1])), int(GOAL_RADIUS*100.), (0, 255, 0), 2)
        # draws humans
        for human in humans_obs:
            draw_oriented_point(world_image, human, (0, 0, 255))
        # shows image
        if not dont_draw:
    ...
    def render(self):
        robot_obs = self.robot
        goal_obs = self.goal
        humans_obs = self.humans
        window_name = "world"
        SocNavEnv.render_obs(robot_obs, goal_obs, humans_obs, window_name, self.world_image)
