...
def evaluate(ann, env, seed, render=False, wait_after_render=False):
    ...
    while True:
        ...
        net_output = ann(torch.tensor(current_obs))
        ...
        # current_obs = next_obs.squeeze(0).squeeze(0).cpu()
        next_obs_ = next_obs
        ...
        with torch.no_grad():
            znew,latent_mu, latent_var ,next_obs = vae(next_obs.to(device)) # (B*T, vsize)
            next_obs =next_obs.unsqueeze(0).to(device)
            
            rnn_input = torch.cat([next_obs, unsqueezed_action.to(device)], dim=-1).float()
            current_obs_X_2_,_, _, hidden = rnn.infer(rnn_input.to(device),hidden)
            current_obs_X_2_ = current_obs_X_2_.squeeze(0)          
            current_obs_X_2_ = current_obs_X_2_[-1, :]
            rnn_input_X_2_ = torch.cat([current_obs_X_2_.unsqueeze(0).unsqueeze(0), unsqueezed_action.to(device)], dim=-1).float()

            _,_, _, hidden = rnn.infer(rnn_input_X_2_,hidden) 
...
if __name__ == '__main__':
    ...
    if len(sys.argv)>2 and sys.argv[1] == '-test':
        ...
        writer_name = f'2StepAheadcmaC_{ENV_NAME}_pop{POPULATION_SIZE}_k{EPISODES_PER_GENERATION}_sigma{SIGMA}_{date_time}'
