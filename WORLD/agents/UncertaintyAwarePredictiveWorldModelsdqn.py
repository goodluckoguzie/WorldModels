...
class DQNAgent:
    def __init__(self, env:gym.Env, config:str, **kwargs) -> None:
        ...
        self.ckpt  = sorted(glob.glob(os.path.join(self.ckpt_dir, 'RobotFrameDatasetsTimestep1window_16', '010robotframe.pth.tar')))[-1] #
    ...
    def get_action(self, current_state, epsilon):
        ...
        if np.random.random() > epsilon:
            # exploit
            with torch.no_grad():
                q = self.model(current_state.unsqueeze(0)).reshape(1, -1).float().to(self.device)
...
###########################################################################################################################                
                ...
                with torch.no_grad():
                    znew,latent_mu, latent_var ,z = self.vae(unsqueezed_z) # (B*T, vsize)
                    z = z.to(self.device)
                    # rnn_input = torch.cat([z, unsqueezed_action.to(self.device)], dim=-1).float()
                ...
                #############################################################################################
                with torch.no_grad():
                    rnn_input = torch.cat([z, action0], dim=-1).float()
                ...
                #############################################################################################
                with torch.no_grad():
                    rnn_input = torch.cat([z, action1], dim=-1).float()
                ...
                #############################################################################################
                with torch.no_grad():
                    rnn_input = torch.cat([z, action2], dim=-1).float()
                ...
                #############################################################################################
                with torch.no_grad():
                    rnn_input = torch.cat([z, action3], dim=-1).float()
                ...
                current_obs = torch.cat((z.unsqueeze(0), hidden_for_action0[0],hidden_for_action1[0],hidden_for_action2[0],hidden_for_action3[0]), -1)
                ...
                if action_discrete == 0:
                    hidden =  hidden_for_action0

                elif action_discrete == 1:
                    hidden =  hidden_for_action1

                elif action_discrete == 2:
                    hidden =  hidden_for_action2

                elif action_discrete == 3:
                    hidden =  hidden_for_action3
                else:
                    raise NotImplementedError
...
#########################################################################################################################################################################
                ...
                with torch.no_grad():
                    ...
                    znew,latent_mu, latent_var ,next_obs = self.vae(unsqueezed_z) # (B*T, vsize)
                ...
                # current_obs = torch.cat((z.unsqueeze(0).unsqueeze(0), hidden[0].unsqueeze(0)),-1)
                # current_obs = torch.cat((z.unsqueeze(0), hidden[0]), -1)
                next_obs = torch.cat((next_obs.unsqueeze(0), hidden_for_action0[0],hidden_for_action1[0],hidden_for_action2[0],hidden_for_action3[0]), -1)
...
if __name__ == "__main__":
    ...
    config = "./configs/Uncertaintypredictiveworldmodelsdqn.yaml"
    input_layer_size = 276 #env.observation_space["goal"].shape[0] + env.observation_space["humans"].shape[0] + env.observation_space["laptops"].shape[0] + env.observation_space["tables"].shape[0] + env.observation_space["plants"].shape[0]+hiddens+hiddens+hiddens+hiddens
    ...
    agent = DQNAgent(env, config, input_layer_size=input_layer_size, run_name="WORLDMODEL")
    agent.train()
