...
if __name__ == "__main__":
    ...
    config = "./configs/1stepaheadpredictivemodeldqn.yaml"
    input_layer_size = 84 #env.observation_space["goal"].shape[0] + env.observation_space["humans"].shape[0] + env.observation_space["laptops"].shape[0] + env.observation_space["tables"].shape[0] + env.observation_space["plants"].shape[0]
    agent = DQNAgent(env, config, input_layer_size=input_layer_size, run_name="DQN_RNN")
    agent.train()
