...
ENV_NAME = 'test'
# Writer name
writer_name = 'Hal{}_{}'.format(ENV_NAME, date_time,)
...
class DQNAgent:
    ...
    def get_action(self, current_state, epsilon):
        ...
        if np.random.random() > epsilon:
            # exploit
            with torch.no_grad():
                q = self.model(torch.from_numpy(current_state).to(self.device).reshape(1, -1).float().to(self.device))
    ...
    def train(self):
        ...
        import cma
        np.random.seed(123)
        es = cma.CMAEvolutionStrategy(len(self.model.get_params()) * [0], 0.1,{'popsize': 100,'seed': 123})

        print("Parameter Size",len(self.model.get_params()) )
        ...
        for iteration in range(10000):
            ...
            if not best or cur_best >= best:
                ...
                d = best_params
                torch.save(self.model.state_dict(), 'half.pt')
                # if i % 50 == 0:
                #     evaluate(ann, env,view=True)

            def save_model(self ):
                torch.save(self.model.state_dict())
            self.save_path = "./models/half"
            # saving model
            if (self.save_path is not None) and ((iteration+1)%5 == 0):
                if not os.path.isdir(self.save_path):
                    os.makedirs(self.save_path)
                try:
                    save_model(self.model ,os.path.join(self.save_path, "episode"+ str(iteration+1).zfill(8) + ".pth"))
                except:
                    print("Error in saving model")
